...
const UpdateService = (props: Props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (index === -1) {
        ...
        form.setFieldsValue({
          auto: {
            checked: temp.autoIncrease[0] !== '' ? true : false,
            start: temp.autoIncrease[0] !== '' ? temp.autoIncrease[0] : '0001',
            end: temp.autoIncrease[1] !== '' ? temp.autoIncrease[1] : '9999',
          },
          maDichVu: temp.maDichVu,
          moTa: temp.moTa,
          prefix: {
            checked: temp.prefix !== '' ? true : false,
            start: temp.prefix !== '' ? temp.prefix : '0001',
          },
          reset: temp.resetEveryDay,
          surfix: {
            checked: temp.surfix !== '' ? true : false,
            start: temp.surfix !== '' ? temp.surfix : '0001',
          },
  ...
  const onFinish = async (values: any) => {
    ...
    let index = services
      ?.filter(item => item.id !== serviceUpdate?.id)
      .findIndex(item => item.maDichVu === service.maDichVu);
    if (index !== -1) {
      Swal.fire({
        title: 'Error!',
  ...
  return (
    <div className='content pl-[24px] pt-[29px] pr-[100px] lg:pr-2 md:pt-10 relative '>
      ...
      <div className='w-full h-full add-content'>
        ...
        >
          ...
          <Space align='center' className=' flex justify-center w-full md:mt-5'>
            <Button
              className='w-[160px] text-primary-400 mr-6 rounded-lg font-bold text-base outline-none border[1.5px] border-solid border-primary-400 bg-primary-50 btn-cancel'
